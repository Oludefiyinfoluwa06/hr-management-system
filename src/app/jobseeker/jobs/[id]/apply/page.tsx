...
export default function JobApplicationPage() {
  ...
  const [resumeCloudUrl, setResumeCloudUrl] = useState<string | null>(null);
    ...
    if (file) {
      ...
      try {
        ...
        const response = await axios.post(
          cloudinaryConfig.API_BASE_URL,
          formData
        );

        const uploadedResumeUrl = response.data.secure_url;
        setResumeCloudUrl(uploadedResumeUrl);
      } catch (uploadError: any) {
        setError(uploadError.response?.data?.error?.message || 'Failed to upload resume.');
  ...
  const handleSubmitApplication = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const applicationData = {
        ...
        coverLetter,
        resumeLink: resumeCloudUrl,
      };

      await axios.post(
        `${config.BASE_API_URL}/job-applications`,
        applicationData,
        {
          headers: {
        ...
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmitApplication} className="space-y-6">
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={isLoading || !resumeCloudUrl}
                className={`px-6 py-2 rounded-lg text-white ${
                  isLoading || !resumeCloudUrl ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
                }`}
              >
